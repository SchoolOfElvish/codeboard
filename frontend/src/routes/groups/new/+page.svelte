      ...
      {#if status === 'failure'}
        {#if Object.keys(errors).length > 0}
          <div role="alert" class="rounded border-l-4 border-red-500 bg-red-50 p-4">
            <strong class="block font-medium text-red-700"> {$_('pages.create_group.something_went_wrong')} </strong> 
      ...
      {:else if status === 'success'}
        <div class="rounded-md bg-green-100 p-4">
          <div class="flex">
            ...
            <div class="ml-3">
               <p class="text-sm font-medium text-green-800">{$_('pages.create_group.success_message')}</p>
            </div>
